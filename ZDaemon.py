...
class ZDaemon(object):
	...
	# List all addr transactions with info
	def listTransactions(self, length, start=0):
		return self._call('listtransactions', "", length, start)

	# list Z addresses
	def listZaddresses(self):
		return self._call('z_listaddresses')

	# list T addresses - DEPRECATED
	def listTaddresses(self):
		return self._call('getaddressesbyaccount', "")

	# List adresses and Bal
